...
public class ExerciseWeightTypeRepository : 
    EmptyEnabledReferenceDataRepository<ExerciseWeightType, ExerciseWeightTypeId, FitnessDbContext>,
    IExerciseWeightTypeRepository
{
    ...
    /// <returns>The exercise weight type if found, Empty otherwise</returns>
    public async Task<ExerciseWeightType> GetByCodeAsync(string code)
    {
        ...
        return await Context.Set<ExerciseWeightType>()
            .AsNoTracking()
            .FirstOrDefaultAsync(x => x.Code == code && x.IsActive) ?? ExerciseWeightType.Empty;
    }
}
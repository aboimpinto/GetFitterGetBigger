Feature: Workout Category Caching
  As a system administrator
  I want workout category data to be cached
  ...
  Scenario: Calling get all workout categories twice should only hit database once
  ...
  @caching @reference-data
  Scenario: Calling get workout category by ID twice should only hit database once
    Given I send a GET request to "/api/ReferenceTables/WorkoutCategories"
    And the response property "workoutCategories" contains at least 1 item

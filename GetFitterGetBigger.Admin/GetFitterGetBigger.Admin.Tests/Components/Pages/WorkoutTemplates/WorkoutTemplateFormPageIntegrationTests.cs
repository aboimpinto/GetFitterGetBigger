...
namespace GetFitterGetBigger.Admin.Tests.Components.Pages.WorkoutTemplates
{
    ...
    public class WorkoutTemplateFormPageIntegrationTests : TestContext
    {
        ...
        public async Task CreateWorkoutTemplate_CompleteWorkflow_Success()
        {
            ...
            {
                // Set name
                var nameInput = cut.Find("input[data-testid='template-name-input']");
                await nameInput.ChangeAsync(new ChangeEventArgs { Value = "Test Template" });

                // Set description
                var descriptionInput = cut.Find("textarea[data-testid='template-description-input']");
                await descriptionInput.ChangeAsync(new ChangeEventArgs { Value = "Test Description" });

                // Set category
                var categorySelect = cut.Find("select[data-testid='template-category-select']");
                await categorySelect.ChangeAsync(new ChangeEventArgs { Value = "category-1" });

                // Set difficulty
                var difficultySelect = cut.Find("select[data-testid='template-difficulty-select']");
                await difficultySelect.ChangeAsync(new ChangeEventArgs { Value = "difficulty-2" });

                // Set duration
                var durationInput = cut.Find("input[data-testid='template-duration-input']");
                await durationInput.ChangeAsync(new ChangeEventArgs { Value = 45 });

                // Submit the form
                var submitButton = cut.Find("button[data-testid='template-submit-button']");
        ...
        public async Task EditWorkoutTemplate_CompleteWorkflow_Success()
        {
            ...
            await cut.InvokeAsync(async () =>
            {
                ...
                // Update description
                var descriptionInput = cut.Find("textarea[data-testid='template-description-input']");
                await descriptionInput.InputAsync(new ChangeEventArgs { Value = "Updated Description" });
        ...
        public async Task CreateWorkoutTemplate_ValidationErrors_ShowsErrors()
        {
            ...
            var validationMessages = cut.FindAll(".text-red-600");
